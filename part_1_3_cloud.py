...
df_movies = spark.read.csv('gs://project5021/data/ml-25m/movies.csv', header=True, inferSchema=True)
...
df_min_diff_pd = df_min_diff.toPandas()

# Draw a bar chart of the best and worst ratings for the bottom 50 genres with the minimum difference
plt.figure(figsize=(10, 6))
plt.bar(df_bottom_50_diff_pd['genres'], df_bottom_50_diff_pd['best_rating'], label='Best Rating', color='g')
plt.bar(df_bottom_50_diff_pd['genres'], df_bottom_50_diff_pd['worst_rating'], label='Worst Rating', color='r')
...
plt.figure(figsize=(10, 6))
plt.plot(df_bottom_50_diff_pd['title'], df_bottom_50_diff_pd['best_rating'], label='Best Rating', marker='o')
plt.plot(df_bottom_50_diff_pd['title'], df_bottom_50_diff_pd['worst_rating'], label='Worst Rating', marker='x')
plt.xticks(rotation=90)
...
# 加载数据集
genome_tags_df = spark.read.csv("gs://project5021/data/ml-25m/genome-tags.csv", header=True, inferSchema=True)
genome_scores_df = spark.read.csv("gs://project5021/data/ml-25m/genome-scores.csv", header=True, inferSchema=True)
...
tags_df = spark.read.csv("gs://project5021/data/ml-25m/tags.csv", header=True, inferSchema=True)
...
df_rating = spark.read.csv('gs://project5021/data/ml-25m/ratings.csv', header=True, inferSchema=True)
df_movie = spark.read.csv('gs://project5021/data/ml-25m/movies.csv', header=True, inferSchema=True)
df_links = spark.read.csv('gs://project5021/data/ml-25m/links.csv', header=True, inferSchema=True)
df_gscores = spark.read.csv('gs://project5021/data/ml-25m/genome-scores.csv', header=True, inferSchema=True)
df_tags = spark.read.csv('gs://project5021/data/ml-25m/genome-tags.csv', header=True, inferSchema=True)
